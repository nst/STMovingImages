...
// adapted from Erica Sadun's MovieMaker class
// https://github.com/erica/useful-things/tree/master/useful%20pack/Movie%20Maker

import AppKit
import AVFoundation

open class STMovieMaker {
        
    fileprivate var height : Int
    fileprivate var width : Int
    fileprivate var framesPerSecond : UInt = 0
    fileprivate var frameCount : UInt = 0
    ...
    open func append(image:NSImage) -> Bool {
    ...
    open func appendImageFromDrawing(_ drawingBlock: (_ context:CGContext) -> ()) -> Bool {
        ...
        let cmTime = CMTimeMake(Int64(frameCount), Int32(framesPerSecond))
    ...
    open func write(_ completionHandler:@escaping (_ path:String) -> ()) {
        guard let existingInput = self.input else {
            print("-- no input -> don't write")
            return
        }
